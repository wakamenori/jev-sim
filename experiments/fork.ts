...
import { errorInfo } from "../src/model-events.ts";
import { runSim } from "../src/run.ts";
import { buildWorld } from "../src/world.ts";
import { diffRuns } from "./diff.ts";
import { type Intervention, validateInterventions } from "./intervention.ts";
...
if (!day || !actor) {
  console.error("usage: pnpm fork --day N --turn N --actor ID [--action KEY] [--support ID]");
  process.exit(1);
}
const turn = Number(arg("--turn"));
if (!Number.isInteger(turn) || turn < 1 || turn > TURNS) throw new Error("--turn 1..3 is required");
const overrides: Intervention[] = (["action", "support"] as const)
  .map((field) => ({ field, value: arg(`--${field}`) }))
  .filter((o): o is { field: Intervention["field"]; value: string } => o.value !== undefined)
...
try {
  validateInterventions(buildWorld(), overrides);
  console.log("--- base");
  const base = await runSim({ label: "base", handleSignals: true });
  console.log("--- fork", JSON.stringify(overrides));
  const fork = await runExperiment({ label: "fork", handleSignals: true }, overrides, basename(base.file));
  ...
  console.log(
    `内面が食い違った人数: ${d.divergedPeople.map((x) => `d${x.day}:${x.people.length}`).join(" ")}`,
  );
