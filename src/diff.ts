import type { Event, Id, Snapshot, World } from "./types.ts";

/** 出来事の中身の比較用の鍵。id は実行ごとにずれるので使わない */
const eventKey = (e: Event) =>
  JSON.stringify([e.day, e.kind, e.actor, e.target ?? null, e.factId ?? null, e.text]);
...
function mindKey(s: Snapshot, id: Id): string {
  ...
  return JSON.stringify([
    m.support,
    Object.entries(m.trust).map(([k, v]) => [k, r(v)]),
    m.knowledge.map((k) => [k.factId, k.source, r(k.belief)]),
    Object.entries(m.opinions ?? {}).map(([k, v]) => [k, r(v)]),
    m.knownKnowers ?? {},
  ]);
}
...
export function diffRuns(base: World, fork: World): RunDiff {
  ...
  const be = base.events.map(eventKey);
  ...
  for (let i = 0; i < Math.max(be.length, fe.length); i++) {
    ...
    if (be[i] !== fk) {
      firstDivergence = { day: (fe[i] ?? base.events[i]).day, base: base.events[i]?.text, fork: fe[i]?.text };
      break;
    }
  }
